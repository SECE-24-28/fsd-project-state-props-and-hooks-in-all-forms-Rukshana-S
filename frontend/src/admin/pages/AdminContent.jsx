...
export default function AdminContent() {
  ...
  const saveFaqList = (list) => { setFaq(list); };
  const addFaq = (f) => saveFaqList([...faq, { ...f, id: Date.now() }]);
  const updateFaq = (id, f) => saveFaqList(faq.map(item => item.id === id ? { ...item, ...f } : item));
  const deleteFaq = (id) => saveFaqList(faq.filter(item => item.id !== id));
