...
import { Sparkles, Truck, RefreshCcw, Gem, Leaf, MessageSquare } from "lucide-react";
        ...
        <div className="container">
          <div className="about-layout">
            ...
            <div className="about-vision-cards" style={{ marginBottom: "70px" }}>
              {[
                { icon: <Sparkles size={24} color="#222" />, title: "100% Authentic", desc: "Every product is sourced directly from verified brand partners and authorized distributors." },
                { icon: <Truck size={24} color="#222" />, title: "Fast Delivery", desc: "Express delivery across India. Free shipping on orders above ₹999." },
                { icon: <RefreshCcw size={24} color="#222" />, title: "Easy Returns", desc: "Hassle-free 15-day return and exchange policy. No questions asked." },
                { icon: <Gem size={24} color="#222" />, title: "Luxury Curation", desc: "Hand-picked styles from India's finest fashion labels and emerging designers." },
                { icon: <Leaf size={24} color="#222" />, title: "Sustainable Fashion", desc: "We prioritize eco-conscious brands and sustainable fabric choices." },
                { icon: <MessageSquare size={24} color="#222" />, title: "24/7 Support", desc: "Our dedicated style consultants are always here to help you find the perfect look." },
              ].map((v, i) => (
                <div key={i} className="vision-card">
                  <div className="vision-icon" style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>{v.icon}</div>
                  <h3>{v.title}</h3>
                  <p>{v.desc}</p>
                </div>
              ))}
